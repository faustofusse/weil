...
/**
 * Vista de solo lectura de un correo: remitente, destinatario y cuerpo
 * completo. Los parecidos y el vínculo con transacciones viven en Fuentes
 * (`SuggestSourcesScreen`, abierta por el matraz), no acá.
 */
@Composable
fun EmailDetailScreen(
    ...
    onNavigateBack: () -> Unit,
    /**
     * The suggestion lab over this receipt: the same flask the notification
     * detail carries. Emails are auto-recorded by a sweep nobody watches, so
     * the only way to see *why* a mail became (or did not become) a
     * transaction is to run the pipeline by hand and read the trace.
     */
    ...
    Scaffold(
        ...
        topBar = {
            AppTopBar(
                ...
                actions = {
                    // The similar-email/transaction lists live one tap away,
                    // on Fuentes (opened by the flask below) \u2014 this screen
                    // stays a plain read of the message.
